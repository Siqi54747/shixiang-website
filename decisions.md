...
## 2026-04-20: Landing Page V7 — Terminal 窗口风格替换 Focus Grid

### 决策

首页视觉重心从"4 栏 Focus Grid"改为"一个模拟 VSCode/macOS Terminal 窗口的组件 (`.thesis-window`)"，呈现 `shixiang-agi-thesis.md` 文件打开的视觉效果。Hero 区域保留。Focus Grid (`components/FocusGrid.tsx`) 删除。

交付 spec：`~/Downloads/shixiang-thesis-section-spec-v3.md`（v3，品牌色确认 + hover 阴影 + WeChat 二维码交互 + CTA 下划线）。Figma 参考 frame："Landing Page — V7 · 主设计（Terminal 窗口版）"。

### 为什么

| 动机 | 说明 |
|---|---|
| 视觉记忆点 | 一众 VC 网站都是纯文本卡片，Terminal 窗口在同类里独一份 |
| 品牌气质 | 呼应"Research first"的研究者/工程师身份 |
| 叙事闭环 | 顶栏"这是什么文件" → 主体"文件内容" → 底栏"文件实时状态"，三段结构自洽 |

### 相关实施细节

**组件**：`components/ThesisWindow.tsx` + `components/ThesisWindow.module.css`
- 用 CSS Module 而非 Tailwind arbitrary values，原因是 spec 直接给了一整套 `:root` CSS 变量 + class 名，原样粘贴比翻译成 Tailwind utilities 可读且少出错
- Spec 里的暖灰色系（`--color-ink: #2C2C2A` / secondary `#5F5E5A` 等）scope 在 ThesisWindow module 内，**不污染全局 Tailwind tokens**；其他页面继续用原 `ink: #17171C` 等

**全局色**：`cream` token 从 `#FAFAF7` 改为 `#FAF6EC`（更暖的米黄）
- 这是 v7 定义的整站背景，不仅 landing
- 其他页面（/reports、/reports/[slug]）视觉上也会暖一档，可接受

**字体**：新增 JetBrains Mono 到 Google Fonts `<link>`，ThesisWindow module 的 `--font-mono` 引用它

**Hero CTA hover 交互**：从"默认无下划线 → hover 出现 crimson 下划线"改为"默认无下划线 → hover 出现 ink 下划线 + 字重加粗"。不变色。
- `font-weight` transition 在静态字重下是瞬间 swap，不是平滑过渡。spec 提到用 `text-shadow` 伪加粗可平滑，但目前 functional 足够，暂不折腾

**Footer `@海外独角兽`**：从纯文本改为可点击按钮 + Popover（`components/WechatPopover.tsx`）
- 不用全屏 modal（modal 只在 Header 的 "Explore Insights" 入口保留，两种入口保留不同交互强度）
- ESC + 点击外部关闭，`aria-haspopup="dialog"` + `role="dialog"`
- Popover 上方带小三角指向 trigger

**Thesis 路由**：新增 `app/thesis/[slug]/page.tsx`，`generateStaticParams` 预生成 4 个 slug（`agi-labs` / `robotics` / `ai-for-science` / `agent-native`），内容先是 "Coming Soon" placeholder 卡片
- 路由命名 `/thesis/<slug>` vs `/<slug>` 尚未拍板（spec §13.2 待确认）
- 先用 `/thesis/<slug>` 因为 spec 的 HTML 示例就是这样

### 暂不做（从 spec P1/P2/P3 延后）
...
- 4 份 thesis 的真实论述正文（目前 `/thesis/<slug>` 是 Coming Soon 占位）
- 可选：二维码高清图 `wechat-qr-haiwaiunicorn.png` 360x360 — 目前 popover 复用已有的 `wechat-qr.jpg`，未来若要换新图再更新
